...
Each admin (identified by Telegram user ID) manages their own isolated set of
accounts. Accounts are stored per-admin in separate JSON files.

Deploy to Railway or any server — runs as a long-polling bot.
"""
...
# Per-admin state
# ---------------------------------------------------------------------------
# Pending login flows: {admin_id: {...state...}}
...
def _admin_dir(admin_id: int) -> Path:
    """Each admin gets their own directory for sessions and data."""
    base = Path(os.getenv("DATA_DIR", ".")) / "admins" / str(admin_id)
...
# ---------------------------------------------------------------------------
@router.message()
async def admin_guard(message: Message) -> None:
    """Fallback — only reached if no other handler matches."""
    pass
...
async def cmd_start(message: Message) -> None:
    ...
    await message.answer(
        ...
        "/login <phone> - Login a new account\n"
        ...
        "/broadcast <list_name> <text> - Broadcast to list\n"
        "/lists - Show broadcast lists\n"
        "/createlist <name> <targets...> - Create list\n"
        "/deletelist <name> - Delete list\n"
        "/editname <alias> <first> [last] - Edit name\n"
        "/editbio <alias> <bio> - Edit bio\n"
        "/editusername <alias> <username> - Edit username\n"
...
async def cmd_accounts(message: Message) -> None:
    ...
    accounts = store.all()
    if not accounts:
        await message.answer("No accounts registered. Use /login <phone> to add one.")
        return
    lines = []
...
async def cmd_login(message: Message) -> None:
    ...
    if not cfg.has_own_api:
        await message.answer("TELEGRAM_API_ID/TELEGRAM_API_HASH not set in .env")
        return
    ...
    await message.answer(
        ...
        "Reply with the code (e.g. /code 12345)"
    )
...
async def cmd_code(message: Message) -> None:
    ...
    try:
        ...
        state["step"] = "2fa"
        await message.answer("2FA enabled. Send password: /password <your_password>")
        return
...
async def cmd_password(message: Message) -> None:
    ...
    if admin_id not in _login_state or _login_state[admin_id].get("step") != "2fa":
        await message.answer("No pending 2FA. Use /login first.")
        return
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2:
    ...
    try:
        ...
        for d in dialogs:
            ...
            if hasattr(entity, "megagroup"):
                ...
                lines.append(f"{getattr(entity, 'title', '?')} | {username} | {chat_type}")
        if not lines:
            await message.answer("No groups/channels found.")
        else:
...
async def cmd_broadcast(message: Message) -> None:
    ...
    bl = ls.get(list_name)
    if not bl:
        await message.answer(f"List '{list_name}' not found. Use /lists to see available.")
        return

    store = _get_store(message.from_user.id)
    accounts = store.all()
    if not accounts:
        await message.answer("No accounts to broadcast from.")
        return
        ...
        try:
            ...
            for target in bl.targets:
                ...
                try:
                    ...
                    acc_results.append("ok")
                except Exception as e:
                    acc_results.append(type(e).__name__)
                await asyncio.sleep(random.uniform(3, 8))
            results.append(f"[{acc.alias}] {'/'.join(acc_results)}")
        except Exception as e:
...
async def cmd_lists(message: Message) -> None:
    ...
    lists = ls.all()
    if not lists:
        await message.answer("No lists. Use /createlist <name> <target1> <target2> ...")
        return
    lines = []
    for bl in lists:
        lines.append(f"{bl.name} ({len(bl.targets)} targets): {', '.join(bl.targets[:5])}")
    await message.answer("\n".join(lines))
...
async def cmd_createlist(message: Message) -> None:
    ...
    targets = parts[2:]
    ls = _get_list_store(message.from_user.id)
    bl = BroadcastList(name=name, targets=targets)
    ls.add(bl)
    await message.answer(f"List '{name}' created with {len(targets)} target(s).")
...
async def cmd_deletelist(message: Message) -> None:
    ...
    await message.answer(f"List '{parts[1].strip()}' deleted.")
...
async def cmd_editname(message: Message) -> None:
    ...
    parts = message.text.split(maxsplit=3)
    if len(parts) < 3:
        await message.answer("Usage: /editname <alias> <first_name> [last_name]")
        return
    store = _get_store(message.from_user.id)
    acc = store.find(parts[1].strip())
    if not acc:
    ...
    try:
        ...
        await message.answer(f"[{acc.alias}] Name updated: {first} {last}".strip())
    except Exception as e:
        await message.answer(f"Error: {type(e).__name__}: {e}")
    finally:
        if client.is_connected():
...
async def cmd_editbio(message: Message) -> None:
    ...
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3:
        await message.answer("Usage: /editbio <alias> <bio text>")
        return
    store = _get_store(message.from_user.id)
    acc = store.find(parts[1].strip())
    if not acc:
...
async def cmd_editusername(message: Message) -> None:
    ...
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3:
        await message.answer("Usage: /editusername <alias> <new_username>")
        return
    store = _get_store(message.from_user.id)
    acc = store.find(parts[1].strip())
    if not acc:
    ...
    try:
        ...
        await message.answer(f"[{acc.alias}] Username updated to @{username}")
    except Exception as e:
        await message.answer(f"Error: {type(e).__name__}: {e}")
    finally:
        if client.is_connected():
...
async def cmd_remove(message: Message) -> None:
    ...
    try:
        ...
        await message.answer(f"[{acc.alias}] Removed (session deleted).")
    except Exception as e:
...
async def run_bot() -> None:
    ...
    if not admin_ids:
        log.warning("ADMIN_IDS not set — no one will be able to use the bot")
